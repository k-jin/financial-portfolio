...
--
  ...
  email varchar(256) not null
	constraint email_ok2 CHECK (email LIKE '%@%'),
  referer varchar(64) not null references rwb_users(name),
--
-- Only root can refer himself
--
  constraint sane_referer_invitee check (name='root' or name<>referer)
);
...
CREATE TABLE rwb_invitee_permissions (
  name VARCHAR(64) NOT NULL references rwb_invitees(name)
	ON DELETE cascade,
  email VARCHAR(256) NOT NULL,
