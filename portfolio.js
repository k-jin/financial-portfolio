...
//document.querySelector('candidate_checkbox').addEventListener('', checkboxHandler);
//document.querySelector('individual_checkbox').addEventListener('CheckboxStateChange', checkboxHandler);
// Global variables
...
// line / data item
//	var d = $("data").getElementById(id).innerHTML;
//	var rows = d.split("\n");
...
//
	if($("#committee_data").html() != null){
		UpdateMapById("committee_data","COMMITTEE");
	}
	if($("#individual_data").html() != null){
		UpdateMapById("individual_data","INDIVIDUAL");
	}
	if($("#candidate_data").html() != null){
		UpdateMapById("candidate_data","CANDIDATE");
	}	
	if($("#opinion_data").html() != null){
...
UpdateCommAggreg = function(inputstr) {
        //console.log(inputstr);
        var money = inputstr.split("|");
        money = money.slice(1,5);
        console.log(money);
        ...
        var dem_money = money[0] + money[1];
        var rep_money = money[2] + money[3];

        $("#dem_contributions").html(dem_money);
        $("#rep_contributions").html(rep_money);

        if(dem_money > rep_money) {
                $("#comm_contributions").css("background-color","blue");
        } else if (rep_money > dem_money) {
                $("#comm_contributions").css("background-color","red");
        }

},


UpdateIndividAggreg = function(inputstr) {
        //console.log(inputstr);
        var money = inputstr.split("||");
        money = money.slice(1,5);
        console.log(money);
        ...
        $("#idem_contributions").html(dem_money);
        $("#irep_contributions").html(rep_money);

        if(dem_money > rep_money) {
                $("#ind_contributions").css("background-color","blue");
        } else if (rep_money > dem_money) {
                $("#ind_contributions").css("background-color","red");
        }
...
// We determine the new bounds of the map
	...
	var what="";
	var what_cycles="";
	if(document.getElementById('committee_checkbox').checked == true){
		what+="committees,";
	}
	
	if(document.getElementById('candidate_checkbox').checked == true){
		what+="candidates,";	
	}
	if(document.getElementById('individual_checkbox').checked == true){
		what+="individuals,";
	}
	
	if(document.getElementById('opinion_checkbox').checked == true){
		what+="opinions,";
	}
	
	var cycles=document.getElementsByClassName('cycle_num');
	for(var c = 0; c < cycles.length; c++){
		if(cycles[c].checked==true){
			what_cycles += cycles[c].id + ",";
		}
	}

	what = what.substring(0, what.length - 1);
	
	if(what_cycles.length == 0){
		what_cycles="1314";
	}
	else{
		what_cycles = what_cycles.substring(0, what_cycles.length - 1);
	}
...
var comm = document.getElementById("committee_checkbox");
if(comm != null) { comm.addEventListener("click", ViewShift);}

var opin = document.getElementById("opinion_checkbox");
if(opin != null) { opin.addEventListener("click",ViewShift); }

var cand = document.getElementById("candidate_checkbox");
if(cand != null) { cand.addEventListener("click",ViewShift); }

var ind = document.getElementById("individual_checkbox");
if(ind != null) { ind.addEventListener("click",ViewShift); }

var cycles = document.getElementsByClassName("cycle_num");
for(var c = 0; c < cycles.length; c++){
	document.getElementById(""+cycles[c].id).addEventListener("click", ViewShift);
}
